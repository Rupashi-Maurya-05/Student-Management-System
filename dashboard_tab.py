...
class DashboardTab:
    ...
    def update_dashboard(self):
        ...
        # ---------------------------
        # ---------------------------
        # 1. Subject-wise Avg per Class / All  
        # ---------------------------
        ...
        axs[0, 0].set_title("Subject-wise Avg (All Classes)" if cls=="All" else "Subject-wise Avg for Class")
        ...
        # ---------------------------
        # ---------------------------
        # 6. Subject-wise Pass vs Fail
        # ---------------------------
        pass_fail = df.groupby('subject_name').apply(
            lambda x: pd.Series({
                'Passed': (x['marks_obtained'] >= PASS_MARK).sum(),
                'Failed': (x['marks_obtained'] < PASS_MARK).sum()
            })
        ).reset_index()

        pass_fail.plot(
            kind='bar', x='subject_name', y=['Passed', 'Failed'],
            stacked=True, color=['#28a745', '#dc3545'], ax=axs[1, 2]
        )
        axs[1, 2].set_title("Subject-wise Pass vs Fail")
        axs[1, 2].set_ylabel("Number of Students")
        axs[1, 2].set_xlabel("Subjects")
